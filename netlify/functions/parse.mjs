// Netlify — POST /api/parse   { pdf?: base64String, text?: string }  ->  profile JSON
...
export default async (req) => {
  ...
  if (pdf && pdf.length > 4_200_000) {
    return json({ error: 'That PDF is too large. Keep it under 3 MB, or paste the text instead.' }, 413);
  }

  try {
    return json(await parseResume({ pdf, text }));
  } catch (err) {
    console.error('parse failed', err);
    return json({ error: 'The resume could not be read. Try again, or paste the text instead.' }, 502);
  }
};
